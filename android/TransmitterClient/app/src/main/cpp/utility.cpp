...
int nl_get_multicast_id(struct nl_sock *sock,
    ...
    ctrlid = genl_ctrl_resolve(sock, const_cast<char *>("nlctrl"));
...
struct ie_info *create_vendor_ie(char *data) {
    ...
    ie->ie_data[0] = 221;
    // Size of IE.
    ie->ie_data[1] = static_cast<unsigned char>(ie_size);
    ...
    for (int i = 5; i < data_size + 5; ++i) {
        ie->ie_data[i] = static_cast<unsigned char>(data[data_idx]);
        ++data_idx;
    }
...
int get_ack(struct nl_msg *msg, void *arg) {
    ...
    // Extract out the IEs.
    if (bss[NL80211_BSS_INFORMATION_ELEMENTS]) {
        unsigned char *ies = reinterpret_cast<unsigned char *>(nla_data(
...
char *read_file(char *file_path) {
    ...
    fread(buffer, sizeof(char), static_cast<size_t>(numbytes), file);
    fclose(file);
...
char **split_data(char *data, int n_ies) {
    ...
    int i;
    char **raw_ies_data = (char **) calloc(static_cast<size_t>(n_ies), (sizeof(char **)));
    ...
    raw_ies_data[i] = (char *) calloc(static_cast<size_t>(len_left), sizeof(char *));
    memcpy(raw_ies_data[i], &data[i * 252], static_cast<size_t>(len_left));
...
// Client side logging.
/*
void log_info(char *wifi_interface, unsigned int data_origin_ts, unsigned int data_origin_ts_usec,
              char *data_origin_ts_str,
              unsigned int data_tx_ts, unsigned int data_tx_ts_usec, int data_size, int attempt,
              int status) {
